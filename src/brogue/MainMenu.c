...
void mainBrogueJunction() {
    ...
    // Begin Oliver's Cheat Hack
    // I have no real clue where this cheat should be inserted where the monster's basic properties are already set,
    // but the architecture procedures did not create the dungeon and the monsters yet, this seems to be a good location.
    // Although it is invoked all the time the player goes to the main junction (may be once a game),
    // But at least it is not invoked 10 times a minute, slowing down the game...
    if (rogue.cheatWeakEels) {
        monsterCatalog[4].maxHP = 6;
        monsterCatalog[4].defense = 0;
        monsterCatalog[4].accuracy = 80;
        monsterCatalog[4].damage.lowerBound = 1;
        monsterCatalog[4].damage.upperBound = 3;
        monsterCatalog[4].damage.clumpFactor = 1;
    }
    // End Oliver's Cheat Hack
